...
public:
    int **array;                       //二维数组指针
    void init();                       //初始化函数
    const void output();               //输出函数
    const void plus(Array a, Array b); //矩阵相加函数
    const void sub(Array a, Array b);  //矩阵相减函数

    Array(); //默认构造含糊，动态申请一个数组，不带参数，需要用户输入行列

    Array(int x, int y);       //构造函数，动态申请矩阵，以行列为参数
    Array(const Array &a);     //拷贝构造函数，申请和复制数组
    ~Array();                  //析构函数，释放空间
...
//矩阵相加函数
const void Array::plus(Array a, Array b)
{
    int i = 0, j = 0;
    for (i = 0; i < this->lines; i++)
    {
...
//矩阵相减函数
const void Array::sub(Array a, Array b)
{
    int i = 0, j = 0;
    for (i = 0; i < this->lines; i++)
    {
